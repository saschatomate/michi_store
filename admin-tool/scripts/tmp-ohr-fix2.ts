...
async function mark(src: string, name: string, x: number, y: number, half: number) {
  ...
  await sharp(buf).composite([{ input: Buffer.from(svg) }]).png().toFile(`${DIR}/ohrfix2-${name}.png`);
}

async function main() {
  const targets: [string, string, number, number, number][] = [
    ["newbase-sophia-frontal-ohr", "sophia-frontal", 658, 425, 110],
    ["newbase-sophia-seitlich-ohr", "sophia-seitlich", 485, 340, 110],
    ["newbase-claire-dreiviertelprofil-ohr", "claire-dvp", 365, 425, 110],
    ["newbase-claire-seitlich-ohr", "claire-seitlich", 370, 435, 110],
    ["newbase-jen-dreiviertelprofil-ohr", "jen-dvp", 665, 405, 110],
    ["newbase-amara-frontal-ohr", "amara-frontal", 690, 400, 110],
  ];
