...
async function mark(src: string, name: string, x: number, y: number, half: number) {
  ...
  await sharp(buf).composite([{ input: Buffer.from(svg) }]).png().toFile(`${DIR}/mark3-${name}.png`);
}

async function main() {
  const targets: [string, string, number, number, number][] = [
    ["newbase-sophia-seitlich-ring", "sophia-seitlich-ring", 590, 378, 150],
    ["newbase-jen-dreiviertelprofil-ring", "jen-dvp-ring", 681, 592, 150],
    ["newbase-jen-seitlich-ring", "jen-seitlich-ring", 586, 566, 150],
    ["newbase-amara-seitlich-ring", "amara-seitlich-ring", 484, 636, 150],
  ];
