...
async function zoom(src: string, name: string, cx: number, cy: number, halfW: number, halfH: number, step: number, scale: number) {
  ...
  for (let x = 0; x <= width; x += step) {
    const sx = x * scale;
    lines.push(`<line x1="${sx}" y1="0" x2="${sx}" y2="${height*scale}" stroke="rgba(255,0,0,0.4)" stroke-width="1"/>`);
    lines.push(`<text x="${sx+2}" y="14" font-size="12" fill="red">${left+x}</text>`);
  }
  for (let y = 0; y <= height; y += step) {
    const sy = y * scale;
    lines.push(`<line x1="0" y1="${sy}" x2="${width*scale}" y2="${sy}" stroke="rgba(0,0,255,0.4)" stroke-width="1"/>`);
  ...
  await sharp(buf).composite([{ input: Buffer.from(svg) }]).png().toFile(`${DIR}/recheck3-${name}.png`);
}

async function main() {
  const targets: [string, string, number, number, number, number, number, number][] = [
    ["newbase-jen-dreiviertelprofil-ring", "jen-dvp-full", 620, 850, 320, 400, 40, 1.4],
    ["newbase-sophia-seitlich-ring", "sophia-seitlich-full", 500, 480, 280, 280, 30, 1.6],
    ["newbase-jen-seitlich-ring", "jen-seitlich-full", 600, 650, 280, 280, 30, 1.6],
    ["newbase-amara-seitlich-ring", "amara-seitlich-full", 600, 750, 280, 280, 30, 1.6],
  ];
