...
async function dotgrid(src: string, name: string, cx: number, cy: number, halfW: number, halfH: number, step: number, scale: number) {
  ...
  await sharp(buf).composite([{ input: Buffer.from(svg) }]).png().toFile(`${DIR}/dotgrid-${name}.png`);
}

async function main() {
  const targets: [string, string, number, number, number, number, number, number][] = [
    ["newbase-jen-seitlich-ring", "jen-seitlich-ring3", 660, 500, 220, 170, 40, 2.2],
    ["newbase-amara-seitlich-ring", "amara-seitlich-ring3", 640, 750, 220, 170, 40, 2.2],
  ];
