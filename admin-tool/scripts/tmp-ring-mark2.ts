...
async function mark(src: string, name: string, x: number, y: number, half: number) {
  ...
  await sharp(buf).composite([{ input: Buffer.from(svg) }]).png().toFile(`${DIR}/mark2-${name}.png`);
}

async function main() {
  const targets: [string, string, number, number, number][] = [
    ["newbase-sophia-seitlich-ring", "sophia-seitlich-ring", 624, 349, 140],
    ["newbase-claire-frontal-ring", "claire-frontal-ring", 697, 874, 140],
    ["newbase-claire-seitlich-ring", "claire-seitlich-ring", 668, 813, 140],
    ["newbase-jen-dreiviertelprofil-ring", "jen-dvp-ring", 585, 679, 140],
    ["newbase-jen-seitlich-ring", "jen-seitlich-ring", 593, 592, 140],
    ["newbase-amara-dreiviertelprofil-ring", "amara-dvp-ring", 704, 785, 140],
    ["newbase-amara-seitlich-ring", "amara-seitlich-ring", 615, 682, 140],
  ];
